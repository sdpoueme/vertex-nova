...
3. **Read each daily note** — call `vault_read` for each one. Collect:
...
6. **Carry forward open tasks** — any `- [ ]` items from the archived dailies should be appended to today's daily note using `vault_daily_append`
