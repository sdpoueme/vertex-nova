...
export async function sendMessage(
  message: string,
  callerAgent: string = "Amazon Quick",
): Promise<string> {
  ...
  if (response.status < 200 || response.status >= 300) {
    throw new Error(
      `Vertex Nova API error: ${response.status} — ${response.body.slice(0, 200)}`,
    );
  }

  try {
    const data = JSON.parse(response.body) as VertexNovaResponse;
