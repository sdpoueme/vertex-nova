...
const CallerAgent = z
  ...
  .describe(
    "Identity of the calling agent. Will be prefixed to all messages sent to Vertex Nova.",
  );
...
export function registerTools(server: McpServer) {
  ...
  server.tool(
    "vertex_health",
    "Check if Vertex Nova is online and responding",
    {},
    async () => {
  ...
  server.tool(
    "vertex_chat",
    "Send a message to Vertex Nova. The message is automatically prefixed with your agent identity. PRIVACY: Do NOT include dollar amounts, account numbers, balances, or any financial data — the message will be rejected.",
    {
      message: z
        .string()
        .describe(
          "Message to send to Vertex Nova. Must be directional/operational only — no financial data.",
        ),
  ...
  server.tool(
    "vertex_investigate_energy",
    "Ask Vertex Nova to investigate energy consumption patterns. Use when a utility bill exceeds budget. PRIVACY: Do NOT include actual bill amounts — only indicate direction (elevated, normal, reduced).",
    {
      utility: z
      ...
      period: z
        ...
        .optional()
        .describe(
          "Time period to investigate (e.g., 'past 2 weeks', 'April', 'last month')",
        ),
      caller_agent: CallerAgent,
    },
  ...
  server.tool(
    "vertex_investigate_security",
    "Ask Vertex Nova to investigate security system patterns (e.g., system not armed, repeated alerts).",
    {
      issue: z
        .string()
        .describe(
          "Description of the security pattern to investigate (e.g., 'system not armed at 11:15 AM on multiple days')",
        ),
      frequency: z
        .string()
        .optional()
        .describe(
          "How often this occurred (e.g., '10 times in 2 weeks', 'daily')",
        ),
      caller_agent: CallerAgent,
    },
  ...
  server.tool(
    "vertex_seasonal_check",
    "Ask Vertex Nova to perform a seasonal readiness check and suggest optimizations for heating/cooling schedules.",
    {
      season: z
      ...
      request: z
        ...
        .optional()
        .describe(
          "Specific request (e.g., 'verify AC is operational', 'optimize heating schedule')",
        ),
      caller_agent: CallerAgent,
    },
    async ({ season, request, caller_agent }) => {
      const seasonTips: Record<string, string> = {
        spring:
          "Spring is here in Sainte-Julie. Can you verify the AC system is operational and suggest an initial cooling schedule? Also check if any smart home automations need seasonal adjustments (e.g., switching from heating to cooling mode, adjusting presence-based schedules).",
        summer:
          "Summer heat is here. Can you review the current AC schedule and optimize it for energy efficiency based on occupancy patterns? Check if any rooms are being cooled unnecessarily and suggest improvements.",
        fall:
          "Fall is approaching. Can you prepare the heating system — verify the furnace/heating is operational, suggest a winter heating schedule optimized for energy savings, and check if weather-based automations are configured?",
        winter:
          "Deep winter in Sainte-Julie. Can you review the current heating schedule for energy efficiency? Check humidity levels, monitor for any heating anomalies, and ensure night mode security is active.",
      };
  ...
  server.tool(
    "vertex_device_status",
    "Ask Vertex Nova for a status report on smart home devices.",
    {
      category: z
    ...
    async ({ category, caller_agent }) => {
      const catStr =
        category === "all" ? "all smart home devices" : `${category} devices`;
  ...
  server.tool(
    "vertex_presence_report",
    "Ask Vertex Nova for presence detection patterns (who's home, arrival/departure trends). PRIVACY: This only reports presence data from network detection — no personal or financial information.",
    {
      period: z
        .string()
        .optional()
